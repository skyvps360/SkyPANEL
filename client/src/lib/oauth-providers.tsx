...
// Official OAuth Provider Icons as SVG Components
export const DiscordIcon: React.FC<{ className?: string }> = ({ className = "h-5 w-5" }) => (
...
// Provider configuration with official colors and icons
...
export const getProviderConfig = (providerName: string) => {
  return oauthProviders[providerName as keyof typeof oauthProviders] || {
    name: providerName,
    icon: null,
    color: '#6B7280',
    bgColor: 'bg-gray-500',
...
export const getProviderIcon = (providerName: string, className?: string) => {
  const config = getProviderConfig(providerName);
  if (config.icon) {
    return <config.icon className={className} />;
  }
  return <span className={className}>🔗</span>;
};

// Helper function to get provider display name
export const getProviderDisplayName = (providerName: string) => {
  const config = getProviderConfig(providerName);
  return config.name;
};
